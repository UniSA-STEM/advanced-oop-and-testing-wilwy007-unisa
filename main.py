'''
File: main.py
Description: ain.py - Demonstration script for the Zoo Management System.
...
# -----------------------------
# Initialize Zoo and Add Animals
# -----------------------------
zoo = Zoo()
for enc in enclosures:
...
# Attempt to move under-treatment animal (should fail)
try:
    zoo.remove_animal(ella)
    zoo.add_animal(ella, new_savannah_enc)  # under treatment, move should fail
...
# Attempt to move Percy to wrong enclosure type (should fail)
try:
    zoo.remove_animal(percy)
    zoo.add_animal(percy, savannah_enc)  # environment/type mismatch
